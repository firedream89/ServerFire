...
{
    qDebug() << "CLIENT TYPE :" << type << " NAME :" << name << "PRIVILEGE :" << privilege;
    qDebug() << "CONNECT TO :" << ip << ":" << port << "PASSWORD :" << passwd;
...
void client::Auth(QString data)
{
    ...
    qDebug() << "Auth : " << data << QCryptographicHash::hash(test.toLatin1(),QCryptographicHash::Sha256).toHex();
    switch(AuthLvl) {
    case unhautorised:
...
void client::ReceiptData(QString data)
{
    qDebug() << "Receipt data from server : " << data;
}
...
bool client::TCPSend(QString data)
{
    ...
    qDebug() << static_cast<quint16>(static_cast<uint>(paquet.size()) - sizeof(quint16));
